...
{
public:
  LidarPwmNode() : rclcpp::Node("lidar_pwm_node")
  {
    ...
    catch (const std::exception& e)
    {
      ...
      // that never spins up.
      RCLCPP_ERROR(get_logger(), "Failed to open PWM device (chip=%d channel=%d base=%s): %s — "
    ...
    timer_ = create_wall_timer(std::chrono::duration<double>(1.0 / control_rate_hz),
                                std::bind(&LidarPwmNode::OnTimer, this));

    last_tick_ = now();
    RCLCPP_INFO(get_logger(), "lidar_pwm_node active: GPIO %d, pwmchip%d/pwm%d", gpio_pin, pwm_chip,
                pwm_channel);
  }
