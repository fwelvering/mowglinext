...
| `dig_obstacle_enabled`, `dig_obstacle_size` | true, 0.60 | L70-71 | subscription only created when enabled; `dig_obstacle_enabled` is injected from the robot template/GUI Settings. `dig_obstacle_size` is the keepout WIDTH; the polygon is biased ahead of the heading latched by `on_odom` (`internal_helpers.hpp` `dig_keepout_polygon`, `kDigKeepoutBehindM` 0.10 m behind → 0.60 m ahead) so the reversed robot is not standing on it (2026-09-10 START_OCCUPIED loop) → Obstacles by `full_system.launch.py` (restart required); size floored at 0.05 |
