...
| `~/add_area` behaviour (validation, id minting, atomic auto-save — **no grid work in the callback**) | `area_manager.cpp` `on_add_area`; the LAWN/NO_GO stamping is LAZY (`classification_dirty_` → `ensure_classification_current_locked`, run by the two readers of the layer: `publish_keepout_mask` and `on_save_map`) |
| Cost of a grid rebuild / polygon rasterisation (one ray cast per grid LINE, edge-driven distance bands — bit-identical to the per-cell definition) | `include/mowgli_map/polygon_raster.hpp` (pure); callers `costmap_filters.cpp` `publish_keepout_mask`, `area_manager.cpp` `apply_area_classifications_locked`; pinned by `test/test_map_replace.cpp` |
| Keepout-mask rebuild timing after a map edit (one rebuild after a `clear_map`/`add_area` burst, `kMapEditSettle` 1.5 s) | `map_server_node.hpp` `kMapEditSettle` / `mask_rebuild_not_before_`, `area_manager.cpp` `defer_mask_rebuild`, `map_server_node.cpp` `on_publish_timer` |
| `areas.dat` durability (temp file + fsync + rename) | `area_manager.cpp` `save_areas_to_file` / `commit_file_atomically` |
...
|------|-------|---------|
| **`ros2/src/mowgli_map/`** | | |
| `CMakeLists.txt` | ~290 | Two static libs (`mowgli_map_lib`, `mowgli_obstacle_tracker_lib`), two executables, six gtest targets |
...
| `polygon_raster.hpp` | ~260 | Package-private, ROS-free rasterisation: `for_each_cell_inside<Scalar>` (line fill), `for_each_cell_near_edges` (edge-driven distance), `covering_range`, `polygon_window` |
...
| `test_map_replace.cpp` | ~520 | GUI map replace: fast keepout mask bit-identical to the per-cell definition (two scenes), line fill == `grid_map::PolygonIterator`, `add_area` + rebuild budgets on a 1687×1516-cell garden, one mask publish after a burst of edits, atomic `areas.dat` |
...
## Pitfalls
- **Never do per-cell × per-vertex work on the grid, and never do grid work inside a service callback.** `map_server_node` spins ONE thread (`main.cpp`), so anything slow blocks every service, the odom-driven boundary monitor and the publish timer. Until 2026-09-20 `publish_keepout_mask` asked `point_in_polygon` + `point_to_polygon_distance` of every area for EVERY cell (5.6 s measured on a desktop core for ~2350 m² / 750 boundary vertices / 10 obstacles at 0.05 m — 0.01 s now; a Pi 4 sharing its cores with Nav2 is several times slower, not measured), `on_add_area` rasterised the whole area list (twice for the new one), and the 1 Hz timer rebuilt the mask between every two `add_area` calls of a GUI "Save map" — which is the field error `CallService /map_server_node/add_area: context deadline exceeded`. Use `polygon_raster.hpp`; if you add a reader of the CLASSIFICATION layer, call `ensure_classification_current_locked()` first.
- **`/keepout_mask` lags a `clear_map` / `add_area` by `kMapEditSettle` (1.5 s) + one timer tick, on purpose**: a half-built map (first area only, everything else lethal) must not reach Nav2 mid-replace. Every OTHER `masks_dirty_` source (promote / discard / dock set / load) still rebuilds on the next tick.
