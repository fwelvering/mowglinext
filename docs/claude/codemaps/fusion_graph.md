...
| GPS payload-staleness backstop (mowglinext#694) — withhold `/gps/fix` while `LocalizationMonitorNode` reports DEAD_RECKONING on `/mowgli/localization/mode_id`, since `gnss_observation_tracker_`'s own receipt-stamp dedup can't see a receiver whose stamp keeps advancing under a frozen payload | `OnGnss` (`callbacks_a.cpp:464`, after the dock block so a genuinely-docked re-anchor isn't gated on it); subscription in `fusion_graph_node_setup_comms.cpp` (`SetupCommunications`); counter `GraphStats::gps_rejects_dead_reckoning` / `GraphManager::RecordGpsRejectDeadReckoning` (`graph_manager.hpp`/`.cpp`), surfaced on `/fusion_graph/diagnostics` |
