...
| `test_dig_skip.cpp` | ~290 | 18 tests: `dig_skip.hpp` (zones, dedup/bound of dig points, drivable runs incl. adjacent swaths / short runs / sparse legacy paths, settle wait) |
| `test_follow_strip_dig.cpp` | ~390 | 4 tick-level tests, fake `/follow_path` + `/navigate_to_pose` servers + real TF buffer: a dig while mowing cancels the goal, waits for the reverse, resumes the same unit past the hole; a later unit is cut at a recorded zone and continued past it; a unit wholly inside a zone is booked without any goal; `EndSession` forgets the dig points |
...
| `test_high_level_status_snapshot.cpp` | ~250 | 12 tests: republished status carries live battery/progress, tree-owned state untouched, `transiting` overrides `sub_state_name` to `"TRANSIT"`, `coverage_plausibility_warning` overrides it to `"COVERAGE_INCOMPLETE"` (issue #680), and `coverage_scan_paused` overrides both with `"SCAN_PAUSED"` |
...
| `battery_charge_tail_current_a` | 0.08 | :394 | XML `{battery_charge_tail_current_a}` → `IsChargeCurrentBelow`, ANDed with `IsBatteryAbove` in BOTH charge wait loops' AUTO exit (issue #759 follow-up — `battery_full_percent` alone reads "full" off raw pack voltage, which stays elevated for the whole CC/CV charge, not just a transient, while the charger is actively pushing current in; mirrors the firmware's own `CHARGE_END_LIMIT_CURRENT`, `board_defaults.h`). Does not gate the manual-resume override (`IsManualResumeRequested`) |
...
- mowglinext#637 phase 2: every per-INDEX map on `BTContext` (`completed_areas`, `attempted_areas`, `area_completed_swaths`, `area_resume_pose_index`, `cross_hatch`, …) is reconciled against `ctx->area_ids` (last-observed `MapArea.id`) on every `GetNextUnmowedArea` probe, `coverage_nodes.cpp` `processResponse()` — the GUI's area edit/delete flow rebuilds the whole area list (`map_server` `on_add_area`), which can re-index a live area mid-session, not only across a restart. A new per-index map added to `BTContext` needs its own `.erase(current_area_idx_)` in that reconciliation block or it silently survives a re-index.
...
- **DIG_OBSTRUCTION exit:** `HomeSequence` needs NO dig-keepout clean-up (the `DiscardNearbyDigKeepouts` node and its map_server service were removed): a dig never stamps the keepout mask, so HOME can always plan. `test/test_dig_obstruction_recovery.cpp` (structural) pins that the tag is gone, that COMMAND_HOME bypasses `DigObstructionGuard`, and that the hold publishes state 1.
- **Dig skip zones (issue #500 protection, Invariant 16):** `include/mowgli_behavior/dig_skip.hpp` (pure: `recordDigPoint`, `insideDigZone`, `nextDrivableRun`, `DigSettleStep`). `behavior_tree_node.cpp` subscribes to `/hardware_bridge/dig_event` (**VOLATILE**) → `BTContext::session_dig_points` + `dig_event_count` (under `context_mutex`; points cleared by `EndSession`, the counter is a monotonic edge detector); `dig_skip_radius_m` is injected by `full_system.launch.py` (`robot_config_util.dig_skip_radius`). `FollowStrip` (`coverage_nodes.cpp`): `stepDigRecovery` (a dig while a goal of ours is active → cancel it, blade off, wait for the bridge's reverse to settle, `DecideUnitResume`, `trimUnitAt` past the zone, blade-off transit), `skipUnitFrontPastDigZones` (called by `sendCurrentSwath` and `onStart`), goal truncation in `sendFollowGoal` (`truncated_at_`: reaching the cut is NOT the end of the unit), `unit_exhausted_by_dig_` (nothing drivable left → unit booked). Tests: `test/test_dig_skip.cpp` (pure), `test/test_follow_strip_dig.cpp` (tick-level, fake `/follow_path` + `/navigate_to_pose` servers). Never move this into a costmap — a keepout under the robot refuses every plan from its own pose.
