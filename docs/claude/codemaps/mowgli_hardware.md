...
| Repeat-dig escalation + operator clear override | `include/mowgli_hardware/dig_escalation.hpp` (`ShouldEscalate`, `CanClearDigEscalation`); node glue `note_dig_for_escalation()` (sets `dig_escalated_anchor_{x,y}_`), `on_clear_dig_escalation()` (`~/clear_dig_escalation`, distance-gated), docking clear (`dig_escalated_` doc comment) |
