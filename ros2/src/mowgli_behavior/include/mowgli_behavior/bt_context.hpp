...
struct BTContext
{
  ...
  /// withLiveStatusFields (status_snapshot.cpp) — TRANSIT wins if both are
  /// true on the same tick. Deliberately NOT auto-cleared on the next area's
  /// completion — a warning from earlier in the session must survive to the
  /// final report, not just flash briefly. Cleared by EndSession so the
  /// next COMMAND_START starts without a stale warning from a previous,
  /// unrelated session.
